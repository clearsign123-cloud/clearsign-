...
function Checkout({ planId, label }) {
    ...
    <div className="space-y-3">
      {!open ? (
        <button
          ...
          className="px-6 py-3 rounded-lg bg-paper text-ink font-medium hover:opacity-90 w-full"
        >
          {label}
        </button>
      ) : (
        <div className="space-y-3">
          <div
            ...
            data-whop-checkout-theme-accent-color="tomato"
          />
          <button
            type="button"
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-4xl mx-auto px-6 py-20 space-y-16">
        <div>
          <p className="text-xs uppercase tracking-widest text-brass">Pricing</p>
          <h1 className="text-4xl mt-2">ClearSign Pricing</h1>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="bg-panel border border-line rounded-xl p-8 space-y-5">
            <div>
              <h3 className="text-2xl">Free</h3>
              <p className="text-muted mt-1">Try every tool, no card required.</p>
            </div>
            <ul className="text-sm space-y-3">
              {FREE_TOOLS.map((t) => (
                <li key={t.name}><span className="text-paper font-medium">{t.name} </span><span className="text-muted"> — {t.desc}</span></li>
              ))}
            </ul>
            <p className="text-xs text-brass pt-2 border-t border-line">2 free uses per tool, per month. No sign-up required.</p>
          </div>

          <div className="bg-redline rounded-xl p-8 space-y-5">
            <div>
              <h3 className="text-2xl">ClearSign Pro</h3>
              <p className="text-paper/80 mt-2 text-sm">Unlimited use of every tool above, plus tools that support you through your whole tenancy. Same access either way — pick how you'd rather pay.</p>
            </div>
            <ul className="text-sm space-y-3">
              {PRO_TOOLS.map((t) => (
                <li key={t.name}><span className="font-medium">{t.name}</span><span className="text-paper/80"> — {t.desc}</span></li>
              ))}
            </ul>
            <div className="grid sm:grid-cols-2 gap-4 pt-4 border-t border-paper/20">
              <div className="bg-ink/40 rounded-lg p-4 space-y-3">
                <div>
                  <p className="text-xs uppercase tracking-widest text-paper/70">Subscribe</p>
                  <p className="text-3xl mt-1">£3.99<span className="text-base">/month</span></p>
                  <p className="text-xs text-paper/70 mt-1">Cancel anytime.</p>
                </div>
                <Checkout planId="plan_JwVD2VX0kHS0r" label="Choose Pro Agent — monthly" />
              </div>

              <div className="bg-ink/40 rounded-lg p-4 space-y-3">
                <div>
                  <p className="text-xs uppercase tracking-widest text-paper/70">One-time</p>
                  <p className="text-3xl mt-1">£5.99</p>
                  <p className="text-xs text-paper/70 mt-1">No subscription. Pay once, keep access.</p>
                </div>
                <Checkout planId="plan_KLZjQmDEaHjqI" label="Choose Pro Agent — one-time" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <h2 className="text-2xl">Coming soon to Pro</h2>
          <div className="grid md:grid-cols-2 gap-4">
            {COMING_SOON.map((t) => (
              <div key={t.name} className="border border-line rounded-lg p-4"><p className="text-paper font-medium">{t.name}</p><p className="text-sm text-muted mt-1">{t.desc}</p></div>
            ))}
          </div>
        </div>
      </main>
