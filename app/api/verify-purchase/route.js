...
export async function GET(req) {
  ...
  if (!apiKey) return Response.json({ valid: false, error: 'Verification not configured yet.' }, { status: 503 });
  try {
    ...
    return Response.json({ valid: false, error: 'Verification request failed.' }, { status: 200 });
  }
}
